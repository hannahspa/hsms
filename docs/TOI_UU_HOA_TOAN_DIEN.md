...
| 09/07 | 🔁 CHỐNG SPAM YÊU CẦU + XÓA DS CŨ | ✅ Xóa 7 yêu cầu chờ duyệt cũ (backup yeu_cau_chinh_sua_bak_xoaduyet_20260709). NV bấm gửi không thấy phản hồi bền → bấm lại → tạo TRÙNG (Tường Uyên 3 yêu cầu giống hệt). FIX: migration 145 (ĐÃ CHẠY VPS) — RPC checkin_xin_dung_ngay_le (trùng theo NV+tháng+năm) + checkin_xin_doi_off (trùng theo đơn OFF) kiểm đã-có-chờ-duyệt → trả cờ 'already' không tạo mới; client giữ 'đã gửi' bền (bỏ reset 5s) + báo rõ. CÒN: Telegram bot thông báo yêu cầu mới cho anh Nam (MIỄN PHÍ — chờ anh tạo bot @BotFather + gửi token/chat_id). Commit a8ff9b6 | a8ff9b6 |
| 09/07 | 🖨 FIX IN HÓA ĐƠN (NV báo) | 🔴→✅ NV: nút "Thanh Toán & In" chỉ thanh toán KHÔNG in, phải vào Danh Sách bấm In riêng. GỐC: `handleConfirmOrder = async () =>` KHÔNG nhận tham số nhưng nút gọi `handleConfirmOrder(true)` → true bị bỏ, hàm KHÔNG có logic in nào → 2 nút "& In"/"Thanh Toán" giống hệt. FIX: nhận `shouldPrint`; sau finalize → `inHoaDonSauChot(oid)` load đơn+items+payments+customer → printReceipt (iframe ẩn không bị popup blocker); lỗi in không hỏng thanh toán (try/catch + báo in lại từ Danh Sách). Tái dùng mẫu handleQuickPrint. Verify: printReceipt chạy (gây print dialog). Commit 99df4da | 99df4da |
| 09/07 | 🎫 NV GÓP Ý: ĐỐI SOÁT SỐ THẺ + HẠN THẺ | ✅ NV lo (a) HSMS "nhảy thêm thẻ" không đối soát được sau cắt MySpa, (b) hạn thẻ khó nhìn. VIỆC 1: script `doi_chieu_so_the_moi_khach.py` đếm SỐ THẺ/khách 2 hệ → 1385 khách khớp, 9 lệch — điều tra: TẤT CẢ thẻ dư là bán MỚI có đơn hàng thật sau ngày MySpa export (Chị Loan 5106 mua 07/07 có đơn; Chị Quyên 5104 có đơn); kiểm toàn hệ: **0 thẻ tạo mới không đơn → HSMS KHÔNG tự nhảy thẻ**, trấn an NV + công cụ đối soát định kỳ. VIỆC 2: hiện NGÀY HẾT HẠN trên thẻ POS (catalog card + CartLine) + CRM TreatmentCard giống MySpa — format dd/mm/yyyy, quá hạn '⚠ tô đỏ', NULL='Không giới hạn'. Verify: thẻ 2989 hiện '⚠ Đã hết hạn: 24/04/2024'. Commit 2cca529+c1df47b | c1df47b |
| 09/07 | 📱 MOBILE DUYỆT TỪ XA (anh Nam đi xa) | 🔴→✅ Anh Nam quản lý từ xa KHÔNG duyệt được yêu cầu NV gửi + vài module không hiển thị hết nội dung trên mobile. GỐC: bảng nhiều cột trên mobile bị ép → nút Duyệt/Từ Chối đẩy RA NGOÀI màn hình (đo: 22/22 nút nằm ngoài viewport), các cột số bị cắt. FIX bằng class chung `.m-card-tbl` (CSS @media 768: table→thẻ dọc, mỗi dòng 1 card, thead/tfoot ẩn, mỗi ô có nhãn data-label, ô hành động class `act` full-width min 46px, ô ẩn class `m-hide`; desktop giữ nguyên bảng): (1) **Xét Duyệt** 6 bảng (OFF/dùng lễ/tăng ca/sửa-xóa/sửa đơn) — 22 nút giờ 100% trong màn; (2) **Bảng Lương Cứng** 9 cột (minWidth 720) → card 8 khoản có nhãn; (3) **Báo Cáo Hoa Hồng/Tour** → card. Verify chrome mobile 390px tràn ngang=0 + desktop 1440px giữ display:table. CÒN bảng khác dùng lại class này khi cần: TabLichDieuDong, KiemSoatChiPhi, BaoCaoNam, marketing. Commit b451992→6247e0b | 6247e0b |
| 05/07 | 🎨 UI LÔ 4 — QUÉT POPUP (đợt Opus) | ✅ Chuẩn hóa 11 popup về primitive: NS TabXetDuyet (Reject+XóaOFF→Modal), TabQuyNgayLe (TíchLũy+SửDụng→Modal), AdminTaoOff→Modal; admin lẻ: DichVu (form→RightPanel, đổi-tên-DM→Modal), CRM thêm khách→Modal md, KhuyenMai→RightPanel headerGrad, ChamSocKhach nhật ký→RightPanel, Kho sửa GD→Modal. GIỮ NGUYÊN CÓ CHỦ ĐÍCH: lightbox zoom ảnh Kho (overlay chuyên dụng), checkin 6 popup (mobile bottom-sheet đã chốt). Verify chrome panel KM. Popup tự vẽ: 51→~45 (admin 15→9). **CÒN LẠI CÓ CHIẾN LƯỢC**: panel NS lương/chấm công (AdminSuaChamCong/TabBangLuong/TabHoSo/TabBaoCaoThuNhap — ĐÃ trượt phải đúng y hệt RightPanel về thị giác, người dùng không phân biệt được; chuyển sang component chỉ lợi code+ESC nhưng RỦI RO cao vì logic tiền lương → để ĐỢT REFACTOR CÓ KIỂM THỬ riêng, KHÔNG làm vội trong đợt UI); internal 22 (nhiều là dropdown/toast/mobile — cần phân loại); POS 8. **--bord** đã fix. Commit d0ad22b | d0ad22b |
| 05/07 | 🎯 ĐỢT "HOÀN THIỆN 100%" C1-C7 | ✅ **C1**: RightPanel có phím ESC (đồng bộ Modal); token TRANSITION duy nhất (.2s cubic-bezier) trong colors.js; T2 `/admin`→redirect dashboard (lưới module = di sản, giữ /admin/home); T3 XÓA trang+route lich-su-nop-tien-mat → chip lọc '🏦 Nộp TM→MB' trong Danh Sách Thu Chi. ✅ **C2 (T5)**: CRM chi tiết khách nút '📜 Lịch sử buổi & thanh toán' mở Modal xl chứa CardHistory dùng chung — verify thẻ 3051 hiện cả buổi 2023 NV đã nghỉ. ✅ **C3 (T1)**: GOM 7 cửa chăm sóc → nhóm menu cha '💗 Chăm Sóc Khách' (admin 7 mục/lễ tân 4 mục), nhóm Marketing rút còn 6 mục thuần; LƯU Ý gom mức MENU — hợp nhất code các trang cron để đợt riêng. ✅ **C4 (T4)**: phát hiện Đối Soát ĐÃ có nút nối Chốt Ngày → giữ 2 trang (gộp full 755 dòng không đáng rủi ro luồng tiền). ✅ **C5**: checkin fmt→fmtGio/fmtNgay; xác nhận các fmt tiền local đã chuẩn ₫ (default '—' chủ đích — hợp lệ). ✅ **C6**: AdminTaoOff → ui/Modal (mẫu nhóm nhan-su); CÒN AdminSuaChamCong (2 popup lồng) + các Tab NS + cai-dat ~30 popup → lô sau. ✅ **C7**: **DESIGN_SYSTEM.md v2** viết lại toàn bộ khớp token thật (8 mục: màu/popup/motion/tiền-ngày/typography/responsive/data/checklist) — thay v1 lỗi thời | (commit) |
| 04-05/07 | 🚀 ĐỢT "2-3-4-5" (anh Nam giao) | ✅ **#2 LỚP 2b RLS BẢNG TIỀN** (mig 144 ĐÃ CHẠY VPS + test 8/8 giả lập JWT): doanh_thu/chi_phi/chuyen_khoan_noi_bo UPDATE+DELETE admin-only; don_hang sửa draft tự do còn lại admin; chi_tiet/thanh_toan theo đơn cha (hsms_don_nhap); Lễ tân vẫn INSERT + đơn nháp; UI đã gate sẵn (DanhSachThuChi isAdmin) → không silent-fail. ✅ **#4 AVATAR STORAGE**: bucket avatars public + script `avatar_base64_to_storage.py` — 9 avatar 35KB base64 → URL (verify 200 + checkin 9 ảnh OK; NV đổi avatar qua checkin vẫn ghi base64 → chạy lại script). ✅ **#3 UI LÔ 4 nhóm Thẻ**: ModalCheckoutBuoi+CardReviewModal → ui/Modal; CardEditModal+ComboEditModal → RightPanel (bodyStyle trắng); DatePicker zIndex 100001 (hết bị Modal che); **FIX --bord**: 16 file dùng var(--bord) nhưng biến CHƯA TỪNG định nghĩa → input không viền từ trước; thêm --bord=#d4c4ad vào :root. CÒN Lô 4: nhan-su Tabs/crm/kho/dich-vu/khuyen-mai/cham-soc-khach. ✅ **#5 BÁO CÁO TỔNG**: nâng cấp NhatKyNgay (/SoThuChi/nhat-ky-ngay) = báo cáo duy nhất — khối CẦN CHÚ Ý (OFF chờ duyệt/yêu cầu sửa chờ/đơn nợ/kho cạn + link xử lý, chỉ hiện hôm nay) + khối NHÂN SỰ (X/9 check-in, chip giờ vào-ra/OFF/chưa vào) + toàn cảnh tài chính sẵn có. Verify chrome data thật (commit c0aafde→68748e9) | 68748e9 |
| 04/07 | 📱 MOBILE ĐỢT 2 | ✅ Quét emulate iPhone 390px các trang đi sâu: bảng lương / lịch ca / lịch hẹn / kho / danh sách thu chi / khuyến mãi = KHÔNG tràn khung (nền đợt 1 + CSS chung đã phủ). FIX 3 chỗ: (1) panel chi tiết thẻ grid 2 cột cứng → auto-fit minmax(min(420px,100%),1fr) — mobile tự 1 cột, desktop giữ 2 cột; (2) khối thanh toán 4 ô → auto-fit 150px; (3) `.crm-list` cuộn ngang trong khung (min-width 760px/dòng) — hết cắt cột Tổng Chi Tiêu. Verify cả desktop 2 cột nguyên vẹn (commit 4a739c9). GHI CHÚ: screenshot CDP hay timeout trên trang admin nặng → khảo sát bằng script đo scrollWidth + a11y snapshot | 4a739c9 |
| 04/07 | ⚡ PERF LỊCH SỬ THẺ | ✅ Anh Nam báo mở thẻ chậm vài giây → gốc: CardHistory chạy 7 query NỐI TIẾP (mỗi round-trip Mỹ↔VPS ~350ms) + avatar base64 ~35KB/NV lặp theo từng dòng. FIX: migration 143 (ĐÃ CHẠY VPS) — RPC `the_lieu_trinh_lich_su(uuid)` trả toàn bộ uses/manual/buy_order/payments/HH/staff trong 1 JSON, avatar mỗi NV 1 lần, chặn anon; FE 1 RPC + cache 60s theo thẻ (mở lại = tức thì). Verify chrome OK (commit 891c29e). GHI CHÚ TỒN ĐỌNG: avatar vẫn base64 trong nhan_vien (tổng 316KB/10 NV) — chuyển Storage URL là việc riêng sau | 891c29e |
| 04/07 | 🎫 CHI TIẾT THẺ v2 — PANEL FULL | ✅ Anh Nam góp ý: (1) "thiếu danh sách như MySpa" → thực tế data ĐỦ (thẻ 5021 hiện đúng 5 lượt commission y hệt MySpa — chỉ do xem trước khi deploy); (2) "popup nhỏ xíu mất cân xứng" → modal hộp giữa 980px chuyển PANEL TRƯỢT PHẢI full chiều cao, bố cục 2 cột lớn (trái thông tin thẻ 5fr / phải lịch sử 8fr); bảng lịch sử thêm CHỨC VỤ dưới tên NV, hết tràn ngang. FIX kèm: fallback tìm đơn mua qua dòng the_moi khi thẻ không gắn don_hang_id (thẻ song song 2 hệ: 5021 → ĐH-44197 tiền mặt 1.690.000đ đã đủ). Verify chrome đầy đủ (commit 560a110) | 560a110 |
| 04/07 | 🎫 CHI TIẾT THẺ ĐẦY ĐỦ NHƯ MYSPA | ✅ Anh Nam so modal thẻ HSMS vs MySpa edit_card "1 trời 1 vực" → xây `the-lieu-trinh/components/CardHistory.jsx` nhúng vào modal AdminTheLieuTrinhPage, 3 khối: (1) Lịch sử SỬ DỤNG BUỔI — ngày/mã đơn/KTV avatar/tiền tour từng buổi (=lịch sử commission MySpa; gộp don_hang_chi_tiet + lich_su_dung_the ghi tay, đơn hủy mờ); (2) THANH TOÁN & CÔNG NỢ — đơn mua/thực thu/các lần trả PTTT/còn nợ, thẻ import MySpa hiện đã-thanh-toán ghi nhận; (3) NHÂN VIÊN BÁN & HOA HỒNG (% + tiền từ nhan_vien_thu_nhap). Verify chrome 2 thẻ: 3500 (2 lượt + tour 20k, Cẩm My + Phương Linh) + 5102 (đơn DH-20260703-015 CK 1tr đủ, HH Hoa Đào 50k 5%). LƯU Ý: don_hang KHÔNG có cột thanh_tien (dùng tong_tien_hang/thuc_thu). Component tái dùng được — có thể gắn thêm vào CRM/POS sau (commit a3568e1) | a3568e1 |
| 04/07 | 🎫 CA CHỊ HỒNG BẠCH + KHÔI PHỤC HẠN THẺ | ✅ NV báo "thẻ THE-LT-2989 MySpa có HSMS không có" → truy vết: HSMS LUÔN CÓ đủ 6 thẻ, buổi khớp MySpa 100% (2989 dùng 7/10 còn 3). Ảnh NV chụp TRƯỚC đơn DH-20260704-004 sáng 04/07 (khách dùng 2 buổi 3051+3500 → DB 9/10 & 5/10, ảnh 8/10 & 4/10 — khớp thời điểm). GỐC NHẦM LẪN: thẻ 2989 hết hạn 24/04/2024 nhưng HSMS hiện 'Không giới hạn' — phát hiện ĐỢT 28/06 XÓA HẠN QUÁ TAY (bak_han_20260628: 4.791 thẻ có hạn → hiện tại chỉ 103; MySpa ghi hạn cụ thể 351 thẻ). FIX: script `sync_han_the_tu_myspa.py` (match mã+SĐT, fallback SĐT+DV duy nhất, DRY-RUN trước) đồng bộ lại 308 thẻ theo cột Ngày hết hạn MySpa (quy tắc anh Nam 22/06), backup bak_han_20260704. Sau sync: 394 thẻ có hạn, het_han 86, thẻ 2989 = het_han đúng. 3 ca không match = thẻ mới mã lệch đã nằm trong 17 ca NV. LƯU Ý UX: POS card hiện 'đã dùng/tổng' (posShared:157) dễ đọc nhầm là 'còn X' — cân nhắc đổi 'Còn X/tổng' | — |
| 04/07 | 🎫 ĐỐI SOÁT THẺ CHỐT (việc #1 cắt MySpa) | ✅ Đối chiếu file MySpa export 03/07 (4.808 thẻ) vs HSMS (4.807): **3.305 nhóm khớp 100% · THIẾU=0 · DƯ=0** — ca "MySpa 3 HSMS 4" NV báo = 2 lỗi data SĐT đã sửa. FIX DATA (có nguồn chân lý): (1) Chị Hiếu SĐT '0Chị0865909697'→'0865909697'; (2) Chị Trâm TRÙNG 2 hồ sơ → gộp thẻ THE-LT-1377 về hồ sơ SĐT 0766655018, hồ sơ rác đánh dấu 'GOP-KH-01616'; (3) thẻ THE-LT-4484 Chị Phương thêm hậu tố '(Bảo hành 1 năm)' khớp MySpa. CÒN 17 ca lệch SỐ BUỔI (13 ca HSMS dư +1 = khách dùng ghi MySpa chưa lên đơn HSMS → NV lên đơn Dùng Thẻ bổ sung có gán KTV; 1 ca Huệ Minh -1 = thẻ 1 buổi THE-LT-5048 chỉ có MySpa đã hết hạn 28/06; 1 ca Chị Nị tổng khớp lệch phân bổ) → file cho NV: `D:\Hannah Spa\Database\DOI_SOAT_THE_CHO_NV_20260704.xlsx` (kèm sheet GHI CHU: Quỳnh Tuyến không SĐT 2 hệ + 5 khách SĐT 11 số cần xin lại). Xác nhận thêm: mã 50xx HSMS ≠ 50xx MySpa (THE-LT-5063 = Chị Tâm HSMS nhưng = Nhật Tân MySpa) — tuyệt đối không so mã. QUY TRÌNH CHỐT: NV xử 17 ca → export MySpa mới → chạy lại script → 0 lệch → CẮT | — |
| 04/07 | 📱 MOBILE ĐỢT 1 | ✅ Nền tảng mobile toàn hệ thống (khảo sát + fix bằng chrome-devtools emulate iPhone 390px): (1) CSS responsive chung hannah-admin.css/modules.css — mod-head wrap (hết tiêu đề bóp dọc), strip KPI 2 cột + số co, bảng cuộn ngang, kpi-value clamp, chips wrap, nút chạm ≥40px; (2) Sổ Thu Chi: BottomNav cũ CHỒNG nav MobileShell → XÓA BottomNav.jsx, thay bằng dải tab pill cuộn ngang đầu trang + FAB nhập nhanh Thu/Chi/CK nổi góc phải; ví 3 thẻ → cuộn ngang snap kiểu app ngân hàng (hết cắt số); (3) POS mobile: 2 cột ép → 1 cột catalog full + nút giỏ nổi 🛒 (đếm dòng + tổng tiền) mở panel đơn toàn màn có nút đóng; (4) AdminApp trang chủ: grid 3/4 cột cứng → auto-fit/auto-fill minmax. Verify mobile: Dashboard/STC/POS/DanhSáchĐơn/CRM/NhânSự + desktop 1440px KHÔNG vỡ. CÒN (đợt 2): bảng CRM/NS xem chi tiết dạng card mobile, RightPanel form dài trên mobile, Lịch Hẹn timeline mobile | (commit) |
