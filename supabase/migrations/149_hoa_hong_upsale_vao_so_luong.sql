-- ═══════════════════════════════════════════════════════════════════════════
-- 149 · HOA HỒNG UPSALE VÀO SỔ LƯƠNG (12/07/2026)
-- Anh Nam soi đơn DH-20260627-018 (chị Hà): dòng dịch vụ upsale có
-- tien_hoa_hong=20k (10% chênh) nhưng SỔ LƯƠNG chỉ có tour — RPC finalize chỉ
-- ghi hoa_hong cho san_pham/the_moi, BỎ QUÊN dich_vu/the_lieu_trinh upsale
-- → KTV mất tiền upsale trong báo cáo thu nhập + lương.
-- (1) Fix finalize: ghi hoa hồng cả dich_vu/the_lieu_trinh có tien_hoa_hong>0.
-- (2) Backfill T6+T7/2026 (lương chưa chốt). T5 GIỮ NGUYÊN — lương T5 đã trả
--     theo commission MySpa, backfill sẽ trả đôi + lệch đối soát.
-- ═══════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.pos_finalize_order(p_don_hang_id uuid, p_trang_thai text, p_giam_gia integer DEFAULT 0, p_vat integer DEFAULT 0, p_con_no integer DEFAULT 0, p_ghi_chu text DEFAULT NULL::text, p_skip_doanh_thu boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    IF v_item.nhan_vien_id IS NOT NULL THEN
      ...
      -- [149] THÊM dich_vu/the_lieu_trinh: hoa hồng UPSALE (tien_hoa_hong trên dòng
      -- dịch vụ) trước đây bị BỎ QUÊN khỏi sổ lương → KTV mất tiền upsale.
      v_comm_amount := CASE
        WHEN v_item.loai_item IN ('san_pham', 'the_moi', 'dich_vu', 'the_lieu_trinh')
  ...
  IF NOT v_is_test THEN
    ...
    LOOP
      ...
      IF v_item.loai_item = 'the_moi' THEN
        ...
        -- [148] Đơn SỬA LẠI: thẻ của đơn có thể đã tồn tại (được giữ khi reopen vì đã
        -- dùng buổi) → KHÔNG tạo trùng. Chỉ tạo khi đơn chưa có thẻ cùng tên.
        IF NOT EXISTS (
          SELECT 1 FROM the_lieu_trinh
          WHERE don_hang_id = p_don_hang_id
            AND ten_dich_vu = COALESCE(v_item.meta->>'tenDichVu', 'The lieu trinh')
        ) THEN
...
-- ── (2) BACKFILL hoa hồng upsale thiếu — CHỈ đơn từ 01/06/2026 ───────────────
INSERT INTO nhan_vien_thu_nhap (
  don_hang_id, don_hang_chi_tiet_id, nhan_vien_id, loai, nguon, ngay,
  doanh_so_tinh, ti_le, so_tien, trang_thai, is_test, ghi_chu
)
SELECT
  ct.don_hang_id, ct.id, ct.nhan_vien_id, 'hoa_hong', 'pos', d.ngay,
  COALESCE(ct.thanh_tien, 0), ct.ti_le_hoa_hong, ct.tien_hoa_hong,
  'phat_sinh', false, 'Backfill HH upsale (mig 149) ' || d.ma_don
FROM don_hang_chi_tiet ct
JOIN don_hang d ON d.id = ct.don_hang_id
WHERE ct.loai_item IN ('dich_vu', 'the_lieu_trinh')
  AND COALESCE(ct.tien_hoa_hong, 0) > 0
  AND ct.nhan_vien_id IS NOT NULL
  AND d.ngay >= '2026-06-01'
  AND d.trang_thai NOT IN ('draft', 'huy')
  AND d.is_test = false
  AND NOT EXISTS (
    SELECT 1 FROM nhan_vien_thu_nhap nt
    WHERE nt.don_hang_chi_tiet_id = ct.id AND nt.loai = 'hoa_hong'
  );
