-- HSMS Phase 2: POS atomic finalize/void
-- Muc tieu:
-- - Chot don xu ly san pham, the lieu trinh cu, the moi, doanh thu, cong no, CRM trong 1 RPC.
-- - Huy don dao nguoc du lieu da sinh ra tu POS khi con an toan.
-- - Hannah Spa chi co 4 PTTT; the_lieu_trinh la dong hang, khong phai PTTT.
...
BEGIN
  ...
  -- Validate truoc khi tao bat ky du lieu phu nao.
  FOR v_item IN
    SELECT *
    FROM don_hang_chi_tiet
    WHERE don_hang_id = p_don_hang_id
  ...
  UPDATE don_hang SET
    ...
    con_no         = v_con_no,
    trang_thai     = CASE
                       WHEN v_con_no = 0 THEN 'da_thanh_toan'
                       ELSE 'no_mot_phan'
                     END,
    ghi_chu        = COALESCE(p_ghi_chu, v_dh.ghi_chu),
    tien_tour      = COALESCE((SELECT SUM(tien_hoa_hong) FROM don_hang_chi_tiet WHERE don_hang_id = p_don_hang_id), 0),
    updated_at     = now()
  WHERE id = p_don_hang_id;
