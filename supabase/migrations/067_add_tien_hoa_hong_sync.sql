-- ============================================================
-- MIGRATION 067: Thêm cột tien_hoa_hong (P3 - giai đoạn 1, KHÔNG vỡ prod)
-- Ngày: 03/06/2026
-- Triệt để khái niệm: bỏ "commission", chỉ còn Tiền Tour / Tiền Hoa Hồng.
--
-- Chiến lược ZERO-DOWNTIME:
--   1) ADD cột tien_hoa_hong + backfill từ tien_commission.
--   2) Trigger đồng bộ 2 chiều: code CŨ ghi tien_commission, code MỚI ghi
--      tien_hoa_hong — trigger luôn giữ 2 cột bằng nhau trong lúc chuyển đổi.
--   3) Recreate pos_finalize_order đọc tien_hoa_hong (trích nguyên văn từ 049).
--   Views vẫn đọc tien_commission (đồng bộ → chạy đúng). Drop tien_commission
--   ở migration 069 SAU khi code mới đã deploy.
-- ============================================================

-- 1) Thêm cột + backfill
ALTER TABLE don_hang_chi_tiet ADD COLUMN IF NOT EXISTS tien_hoa_hong integer DEFAULT 0;
UPDATE don_hang_chi_tiet SET tien_hoa_hong = COALESCE(tien_commission, 0)
WHERE tien_hoa_hong IS DISTINCT FROM COALESCE(tien_commission, 0);

-- 2) Trigger đồng bộ tien_commission <-> tien_hoa_hong (giai đoạn chuyển đổi)
CREATE OR REPLACE FUNCTION trg_sync_hoa_hong_commission()
RETURNS trigger LANGUAGE plpgsql AS $SYNC$
BEGIN
  IF NEW.tien_hoa_hong IS NULL AND NEW.tien_commission IS NOT NULL THEN
    NEW.tien_hoa_hong := NEW.tien_commission;
  ELSIF NEW.tien_commission IS NULL AND NEW.tien_hoa_hong IS NOT NULL THEN
    NEW.tien_commission := NEW.tien_hoa_hong;
  ELSIF COALESCE(NEW.tien_hoa_hong,0) <> COALESCE(NEW.tien_commission,0) THEN
    -- 2 giá trị khác nhau → ưu tiên tien_hoa_hong (code mới)
    NEW.tien_commission := NEW.tien_hoa_hong;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_sync_hoa_hong_commission
  BEFORE INSERT OR UPDATE ON don_hang_chi_tiet
  FOR EACH ROW EXECUTE FUNCTION trg_sync_hoa_hong_commission();

-- 3) Recreate pos_finalize_order đọc tien_hoa_hong (trích nguyên văn từ migration 049)
...
BEGIN
  ...
  LOOP
    IF v_item.nhan_vien_id IS NOT NULL THEN
      ...
      -- Sau migration 048: tien_hoa_hong đã chứa đủ data
      v_comm_amount := CASE
        WHEN v_item.loai_item IN ('san_pham', 'the_moi')
        THEN COALESCE(v_item.tien_hoa_hong, 0)
        ELSE 0
      END;
