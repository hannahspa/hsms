-- ═══════════════════════════════════════════════════════════════════════════
-- 148 · SỬA ĐƠN CÓ THẺ ĐÃ DÙNG BUỔI — GIỮ THẺ, KHÔNG CHẶN, KHÔNG DUPE (12/07)
-- Anh Nam: sửa đơn bán thẻ (thẻ đã dùng 1/5 buổi) → lỗi đỏ "The moi cua don nay
-- da duoc su dung — can doi soat thu cong truoc khi sua" → không sửa được gì.
-- Fix 2 đầu:
--   (1) pos_reopen_order: BỎ chặn; khi mở lại GIỮ NGUYÊN thẻ đã dùng buổi
--       (chỉ xóa thẻ chưa dùng để finalize tạo lại như cũ).
--   (2) pos_finalize_order: khi chốt lại, CHỈ tạo thẻ nếu đơn chưa có thẻ cùng
--       tên → thẻ giữ lại không bị nhân đôi, buổi đã dùng của khách còn nguyên.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- [148] BỎ chặn "thẻ đã dùng buổi": giờ mở lại đơn sẽ GIỮ NGUYÊN thẻ đã dùng
  -- (không xóa/tạo lại) — finalize [148] sẽ không tạo thẻ trùng. Anh Nam 12/07.
  ...
  IF NOT v_is_test THEN
    ...
    DELETE FROM the_lieu_trinh WHERE don_hang_id = p_don_hang_id AND COALESCE(so_buoi_da_dung, 0) = 0;  -- [148] giữ thẻ đã dùng buổi
