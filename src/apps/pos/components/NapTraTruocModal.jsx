...
export default function NapTraTruocModal({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,.5)' }} onClick={handleBackdropClick}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 16 }}>
          <div style={{ width: 38, height: 38, borderRadius: '50%', flexShrink: 0, background: 'rgba(201,169,110,.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>👛</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 15, color: C.ink, lineHeight: 1.2 }}>Nạp Ví Trả Trước</div>
            <div style={{ fontSize: 12, color: C.ink2, marginTop: 2 }}>{customer?.ho_ten}{customer?.so_dien_thoai ? ` · ${customer.so_dien_thoai}` : ''}</div>
          </div>
        </div>
        ...
        {amountValue > 0 && (
          <div style={{ background: 'rgba(45,122,79,.06)', border: '1px solid rgba(45,122,79,.22)', borderRadius: 8, padding: '8px 12px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
