  ...
  return (<div style={{padding:'22px 24px'}}>
    ...
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        ...
        <div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Báo Cáo Ngày</div><div style={{fontSize:12,color:'var(--ink3)'}}>{DAYS[d.getDay()]}, {formatDateInput(ngay)}{isToday?' · Hôm nay':''}</div></div>
      </div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}>
        <button onClick={()=>change(-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button>
...
function SubBaoCaoTuan({ onBack }) {
  ...
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Báo Cáo Tuần</div><div style={{fontSize:12,color:'var(--ink3)'}}>{formatDateInput(week.start)} - {formatDateInput(week.end)}</div></div></div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}>
        <button onClick={()=>changeWeek(-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button><span style={{fontSize:13,fontWeight:600,padding:'0 8px'}}>Tuần này</span><button onClick={()=>changeWeek(1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>›</button>
      </div>
    </div>
    <div className="strip" style={{gridTemplateColumns:'repeat(3,1fr)',marginBottom:16}}>
...
// ════════════════ SUB: BÁO CÁO THÁNG ════════════════
function SubBaoCaoThang({ onBack }) {
  ...
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Báo Cáo Tháng</div><div style={{fontSize:12,color:'var(--ink3)'}}>Biểu đồ và phân tích chuyên sâu</div></div></div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}>
        <button onClick={()=>chg(-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button>
    ...
    <div className="strip" style={{gridTemplateColumns:'repeat(4,1fr)',marginBottom:16}}>
      ...
      <div className="it"><div className="l">LỢI NHUẬN</div><div className="v" style={{color:loiNhuan>=0?'#426a2c':'#843a23'}}>{formatCurrency(loiNhuan)}</div><div className="d">Biên LN {bienLN}%{data.prevThu>0?<span style={{color:data.pctThu-data.pctChi>=0?'#426a2c':'#843a23',marginLeft:4}}>· {data.pctThu-data.pctChi>=0?'↑':'↓'}{Math.abs(data.pctThu-data.pctChi)}% biên LN</span>:''}</div></div>
    ...
    <div className="card" style={{marginBottom:16}}>
      ...
      <div className="card-b">
        <div style={{display:'flex',alignItems:'flex-end',gap:1,height:160,padding:'0 4px'}}>
          {data.days.map((d,i)=>{
            const hThu=(d.thu/maxV)*140;const hChi=(d.chi/maxV)*140
            return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end',height:'100%',position:'relative'}}>
              {d.thu>0&&<div title={`${formatDateInput(d.date)}: Thu ${formatCurrency(d.thu)}`} style={{width:'100%',maxWidth:7,height:Math.max(hThu,1),background:'#6e8a5e',borderRadius:'2px 2px 0 0',opacity:.6,cursor:'pointer'}}/>}
              {d.chi>0&&<div title={`${formatDateInput(d.date)}: Chi ${formatCurrency(d.chi)}`} style={{width:'100%',maxWidth:7,height:Math.max(hChi,1),background:'#b85a4a',borderRadius:'2px 2px 0 0',opacity:.6,cursor:'pointer',marginTop:1}}/>}
              {i%3===0&&<span style={{fontSize:7,color:'var(--ink3)',marginTop:2}}>{d.day}</span>}
              {/* Hiện giá trị cho ngày có DT cao */}
              {d.thu>maxV*0.7&&<span style={{position:'absolute',bottom:hThu+4,fontSize:8,fontWeight:700,color:'#426a2c',whiteSpace:'nowrap'}}>{(d.thu/1000000).toFixed(1)}M</span>}
            </div>)
          })}
        </div>
      </div>
    ...
    <div className="fin-grid" style={{marginBottom:16, gridTemplateColumns:'1fr 1fr'}}>
      <div className="card">
        ...
        <div className="card-b">
          <div className="donut-wrap">
            <DonutChart segments={[{v:data.tienMat,c:'#c9a96e'},{v:data.ck,c:'#a87366'},{v:data.qt,c:'#6e8a5e'},{v:data.tt,c:'#8a6a6e'}]} total={data.thucThu+data.tt||1}/>
            <div className="donut-leg">
              {[{l:'Tiền Mặt',v:data.tienMat,c:'#c9a96e'},{l:'Chuyển Khoản',v:data.ck,c:'#a87366'},{l:'Quẹt Thẻ',v:data.qt,c:'#6e8a5e'},{l:'Thẻ Trả Trước',v:data.tt,c:'#8a6a6e'}].map((s,i)=>(<div className="row" key={i}><span className="sw" style={{background:s.c}}/><span>{s.l}</span><b>{formatCurrency(s.v)}</b><span className="pct">{Math.round(s.v/((data.thucThu+data.tt)||1)*100)}%</span></div>))}
            </div>
          </div>
        </div>
      </div>
      <div className="card">
        ...
        <div className="card-b">
          {data.cpByNhom.length===0?<div style={{textAlign:'center',padding:30,color:'var(--ink3)'}}>Không có chi phí</div>:
          <div className="donut-wrap">
            <DonutChart segments={data.cpByNhom.map((n,i)=>({v:n.total,c:['#c9a96e','#a87366','#6e8a5e','#8a6a6e','#5a8db8','#b85a4a'][i%6],l:n.ten}))} total={data.tongChi||1}/>
            <div className="donut-leg">
              {data.cpByNhom.map((n,i)=>(<div className="row" key={i}><span className="sw" style={{background:['#c9a96e','#a87366','#6e8a5e','#8a6a6e','#5a8db8','#b85a4a'][i%6]}}/><span>{n.icon} {n.ten}</span><b>{formatCurrency(n.total)}</b><span className="pct">{Math.round(n.total/(data.tongChi||1)*100)}%</span></div>))}
            </div>
          </div>}
        </div>
      </div>
    ...
    {/* Bảng Phân Tích Lợi Nhuận — full width */}
    <div className="card" style={{marginBottom:16}}>
...
// ════════════════ PLACEHOLDER ════════════════
function SubPlaceholder({ title, onBack }) {
  return <div style={{padding:40}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36,marginBottom:20}}><I.ArrowLeft style={{width:16,height:16}}/></button><h2 style={{fontFamily:'var(--serif)'}}>{title}</h2><p style={{color:'var(--ink3)',marginTop:12}}>Đang xây dựng...</p></div>
}

// ════════════════ SUB: BÁO CÁO NĂM ════════════════
function SubBaoCaoNam({ onBack }) {
  ...
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Báo Cáo Năm</div><div style={{fontSize:12,color:'var(--ink3)'}}>Tổng kết 12 tháng · So sánh năm trước</div></div></div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}><button onClick={()=>setNam(n=>n-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button><span style={{fontSize:16,fontWeight:700,fontFamily:'var(--serif)',minWidth:70,textAlign:'center'}}>{nam}</span><button onClick={()=>setNam(n=>n+1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>›</button></div>
    </div>
    <div className="strip" style={{gridTemplateColumns:'repeat(4,1fr)',marginBottom:16}}>
...
function SubBaoCaoDongTien({ onBack }) {
  const now=getNowVN();const [thang,setThang]=useState(now.getMonth()+1);const [nam,setNam]=useState(now.getFullYear())
  const [data,setData]=useState(null);const [loading,setLoading]=useState(true)
  ...
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Báo Cáo Dòng Tiền</div><div style={{fontSize:12,color:'var(--ink3)'}}>Lưu chuyển tiền tệ — {MONTHS[thang]} {nam}</div></div></div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}><button onClick={()=>chg(-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button><span style={{fontSize:14,fontWeight:700,fontFamily:'var(--serif)',minWidth:140,textAlign:'center'}}>{MONTHS[thang]} {nam}</span><button onClick={()=>chg(1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>›</button></div>
    </div>
    {/* Bảng tổng hợp */}
...
function SubPhanTichDoanhThu({ onBack }) {
  const now=getNowVN();const [thang,setThang]=useState(now.getMonth()+1);const [nam,setNam]=useState(now.getFullYear())
  const [data,setData]=useState(null);const [loading,setLoading]=useState(true)
  ...
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Phân Tích Doanh Thu</div><div style={{fontSize:12,color:'var(--ink3)'}}>Theo hình thức thu — {MONTHS[thang]} {nam}</div></div></div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}><button onClick={()=>chg(-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button><span style={{fontSize:14,fontWeight:700,fontFamily:'var(--serif)',minWidth:140,textAlign:'center'}}>{MONTHS[thang]} {nam}</span><button onClick={()=>chg(1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>›</button></div>
    </div>
    <div className="strip" style={{gridTemplateColumns:'repeat(3,1fr)',marginBottom:16}}>
...
function SubLichSuNopTienMat({ onBack }) {
  const now=getNowVN();const [thang,setThang]=useState(now.getMonth()+1);const [nam,setNam]=useState(now.getFullYear())
  const [data,setData]=useState(null);const [loading,setLoading]=useState(true)
  ...
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Lịch Sử Nộp Tiền Mặt</div><div style={{fontSize:12,color:'var(--ink3)'}}>Tiền Mặt → MB Bank từng ngày — {MONTHS[thang]} {nam}</div></div></div>
      <div style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',border:'1px solid var(--line)',borderRadius:14,padding:4}}><button onClick={()=>chg(-1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>‹</button><span style={{fontSize:14,fontWeight:700,fontFamily:'var(--serif)',minWidth:140,textAlign:'center'}}>{MONTHS[thang]} {nam}</span><button onClick={()=>chg(1)} className="icon-btn" style={{width:32,height:32,border:'none',background:'var(--bg2)'}}>›</button></div>
    </div>
    <div className="strip" style={{gridTemplateColumns:'repeat(3,1fr)',marginBottom:16}}>
...
function SubCongNoKhachHang({ onBack }) {
  const [data,setData]=useState(null);const [loading,setLoading]=useState(true)
  useEffect(()=>{setLoading(true);Promise.all([supabase.from('cong_no_khach_hang').select('*').order('ngay',{ascending:false}).limit(50),supabase.from('khach_hang').select('id,ho_ten,so_dien_thoai')]).then(([rCN,rKH])=>{const cnList=rCN.data||[];const khMap={};(rKH.data||[]).forEach(k=>{khMap[k.id]=k});const khNo={};cnList.forEach(cn=>{const khId=cn.khach_hang_id;if(!khNo[khId])khNo[khId]={kh:khMap[khId]||{ho_ten:'?',so_dien_thoai:''},no:0};if(cn.loai==='phat_sinh')khNo[khId].no+=cn.so_tien||0;else if(cn.loai==='thanh_toan')khNo[khId].no-=cn.so_tien||0});const list=Object.values(khNo).filter(x=>x.no>0).sort((a,b)=>b.no-a.no);const tongNo=list.reduce((s,x)=>s+x.no,0);setData({list,tongNo});setLoading(false)}).catch(()=>setLoading(false))},[])
  if(loading)return<div style={{padding:60,textAlign:'center',color:'var(--ink3)'}}>Đang tải...</div>
  return (<div style={{padding:'22px 24px'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={onBack} className="icon-btn" style={{width:36,height:36}}><I.ArrowLeft style={{width:16,height:16}}/></button><div><div style={{fontFamily:'var(--serif)',fontSize:26,fontWeight:700,color:'var(--ink)'}}>Công Nợ Khách Hàng</div><div style={{fontSize:12,color:'var(--ink3)'}}>Danh sách khách hàng còn nợ</div></div></div>
    </div>
...
export default function BaoCaoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...months.flatMap(m => [
        supabase.from('doanh_thu').select('so_tien').gte('ngay', m.start).lte('ngay', m.end),
      ...
      setChartData(months.map((m, i) => {
        const thu = (results[i * 2].data || []).reduce((s, r) => s + (r.so_tien || 0), 0)
        const chi = (results[i * 2 + 1].data || []).reduce((s, r) => s + (r.so_tien || 0), 0)
        return { label: m.label, thu, chi }
      }))
      setViList(results[results.length - 1].data || [])
      setLoading(false)
    }).catch(() => setLoading(false))
  }, [])

  if (activeReport === 'ngay') return <SubBaoCaoNgay onBack={() => setActiveReport(null)} />
  if (activeReport === 'tuan') return <SubBaoCaoTuan onBack={() => setActiveReport(null)} />
  if (activeReport === 'thang') return <SubBaoCaoThang onBack={() => setActiveReport(null)} />
  if (activeReport === 'nam') return <SubBaoCaoNam onBack={() => setActiveReport(null)} />
  if (activeReport === 'dongtien') return <SubBaoCaoDongTien onBack={() => setActiveReport(null)} />
  if (activeReport === 'chiphi') return <SubPhanTichChiPhi onBack={() => setActiveReport(null)} />
  if (activeReport === 'doanhthu') return <SubPhanTichDoanhThu onBack={() => setActiveReport(null)} />
  if (activeReport === 'noptienmat') return <SubLichSuNopTienMat onBack={() => setActiveReport(null)} />
  if (activeReport === 'congno') return <SubCongNoKhachHang onBack={() => setActiveReport(null)} />
  ...
  return (
    <div style={{ padding: '22px 24px', flex: 1, overflow: 'auto' }}>
      <div className="mod-head" style={{ marginBottom: 20 }}>
        <div><div className="ttl">Báo Cáo</div><div className="sub">{user?.ho_ten || 'Admin'} · Tổng tài sản: {formatCurrency(totalAsset)}</div></div>
      </div>
        ...
        <div className="card-b">
          {loading ? <div style={{ textAlign: 'center', padding: 40, color: 'var(--ink3)' }}>Đang tải...</div> : (
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 20, height: 200, padding: '0 20px' }}>
              {chartData.map((d, i) => {
                const hThu = (d.thu / maxVal) * 170
                const hChi = (d.chi / maxVal) * 170
                const isLast = i === chartData.length - 1
                return (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 170 }}>
                      <div style={{ width: 22, height: Math.max(hThu, 2), borderRadius: '6px 6px 0 0', background: isLast ? 'var(--grad-gold)' : '#6e8a5e', opacity: isLast ? 1 : 0.6, position: 'relative' }} title={`Thu: ${formatCurrency(d.thu)}`} />
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-h"><div className="card-t"><div className="arch-i"><I.Calendar style={{ width: 13, height: 13, color: '#8a6a52' }} /></div><h3>Báo Cáo Theo Kỳ</h3></div></div>
        <div className="card-b">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {[{ icon: '📋', title: 'Báo Cáo Ngày', desc: 'Chi tiết thu chi từng ngày', r: 'ngay' }, { icon: '📅', title: 'Báo Cáo Tuần', desc: 'Tổng hợp 7 ngày', r: 'tuan' }, { icon: '📆', title: 'Báo Cáo Tháng', desc: 'Biểu đồ và phân tích', r: 'thang' }, { icon: '🗓️', title: 'Báo Cáo Năm', desc: 'Tổng kết 12 tháng', r: 'nam' }].map((c, i) => (
              <button key={i} onClick={() => setActiveReport(c.r)} style={{ background: 'var(--bg2)', border: '1px solid var(--line)', borderRadius: 14, padding: '20px 18px', cursor: 'pointer', textAlign: 'center', fontFamily: 'inherit' }}>
                <div style={{ fontSize: 32, marginBottom: 10 }}>{c.icon}</div>
                <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--ink)', fontFamily: 'var(--serif)', marginBottom: 4 }}>{c.title}</div>
                <div style={{ fontSize: 11, color: 'var(--ink3)', fontFamily: 'var(--sans)' }}>{c.desc}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="card" style={{ marginBottom: 40 }}>
        <div className="card-h"><div className="card-t"><div className="arch-i"><I.Tag style={{ width: 13, height: 13, color: '#8a6a52' }} /></div><h3>Phân Tích</h3></div></div>
        <div style={{ padding: 0 }}>
          {[{ icon: '💵', title: 'Báo Cáo Dòng Tiền', desc: 'Lưu chuyển tiền tệ', r: 'dongtien' }, { icon: '📉', title: 'Phân Tích Chi Phí', desc: 'Theo nhóm & hạng mục', r: 'chiphi' }, { icon: '📈', title: 'Phân Tích Doanh Thu', desc: 'Theo hình thức thu', r: 'doanhthu' }, { icon: '🏦', title: 'Lịch Sử Nộp Tiền Mặt', desc: 'TM → MB Bank từng ngày', r: 'noptienmat' }, { icon: '👤', title: 'Công Nợ Khách Hàng', desc: 'Ai còn nợ · Bao nhiêu', r: 'congno' }].map((item, i) => (
            <button key={i} onClick={() => setActiveReport(item.r)} style={{ display: 'flex', alignItems: 'center', gap: 14, width: '100%', padding: '16px 20px', background: 'none', border: 'none', borderBottom: i < 3 ? '1px solid var(--line)' : 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--bg2)', border: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{item.icon}</div>
              <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: 14, color: 'var(--ink)', fontFamily: 'var(--sans)' }}>{item.title}</div><div style={{ fontSize: 11, color: 'var(--ink3)', marginTop: 2, fontFamily: 'var(--sans)' }}>{item.desc}</div></div>
              <span style={{ color: 'var(--champagne)', fontSize: 20 }}>›</span>
            </button>
          ))}
        </div>
      </div>
