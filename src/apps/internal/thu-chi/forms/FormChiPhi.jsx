                  ...
                  return (
                    <div key={l.key} style={{ marginBottom: 8 }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 64px 96px 90px 28px', gap: 6, alignItems: 'center' }}>
                        ...
                        <input value={fmtN(rawN(l.gia_don_vi))} onChange={e => setKhoLine(l.key, { gia_don_vi: rawN(e.target.value) || '' })} placeholder="0₫" style={{ padding: '8px 8px', borderRadius: 8, border: '1px solid var(--line2)', fontSize: 12.5, textAlign: 'right', background: 'var(--surface2)', color: 'var(--ink)', outline: 'none', fontFamily: 'var(--sans)' }} />
                        <div style={{ textAlign: 'right', fontWeight: 700, fontSize: 12.5, color: thanh > 0 ? 'var(--ink)' : 'var(--ink3)', fontFamily: 'var(--sans)' }}>{thanh > 0 ? new Intl.NumberFormat('vi-VN').format(thanh) + '₫' : '—'}</div>
