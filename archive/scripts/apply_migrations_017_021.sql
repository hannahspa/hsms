-- ============================================================
-- HSMS: Apply Migrations 017 → 021 (Phase 2 POS)
-- Paste toàn bộ file này vào Supabase SQL Editor → Run
-- Tất cả lệnh ALTER đều có IF NOT EXISTS — chạy lại an toàn.
-- ============================================================

-- ── 017 & 018: is_test cho don_hang ─────────────────────────
ALTER TABLE don_hang
  ADD COLUMN IF NOT EXISTS is_test boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_don_hang_is_test ON don_hang(is_test);

-- ── 020: tach tien_tour / tien_commission tren dong hang ────
...
SET
  ...
  tien_commission = CASE
    ...
    ELSE 0
  END
WHERE COALESCE(tien_tour, 0) = 0 AND COALESCE(tien_commission, 0) = 0;

-- ── 021: Bang thu nhap nhan vien per order line ─────────────
CREATE TABLE IF NOT EXISTS nhan_vien_thu_nhap (
  ...
  trang_thai           text NOT NULL DEFAULT 'phat_sinh'
                         CHECK (trang_thai IN ('phat_sinh', 'doi_soat', 'da_chot', 'da_tra', 'huy')),
...
-- Backfill du lieu cu vao nhan_vien_thu_nhap
...
SELECT
  ...
  x.loai, 'pos', dh.ngay,
  COALESCE(dhct.thanh_tien, 0), dhct.ti_le_hoa_hong, x.so_tien,
  CASE WHEN dh.trang_thai = 'huy' THEN 'huy' ELSE 'phat_sinh' END,
  COALESCE(dh.is_test, false), 'Backfill tu POS'
FROM don_hang_chi_tiet dhct
JOIN don_hang dh ON dh.id = dhct.don_hang_id
CROSS JOIN LATERAL (
  VALUES
    ('tour'::text,       CASE WHEN dhct.loai_item IN ('dich_vu','the_lieu_trinh')
                              THEN COALESCE(NULLIF(dhct.tien_tour,0), dhct.tien_hoa_hong, 0) ELSE 0 END),
    ('commission'::text, CASE WHEN dhct.loai_item IN ('san_pham','the_moi')
                              THEN COALESCE(NULLIF(dhct.tien_commission,0), dhct.tien_hoa_hong, 0) ELSE 0 END)
) AS x(loai, so_tien)
WHERE dhct.nhan_vien_id IS NOT NULL AND COALESCE(x.so_tien, 0) > 0
ON CONFLICT DO NOTHING;

-- ── 021 FINAL: pos_finalize_order (atomic, is_test, tach tour/commission) ──
...
BEGIN
  ...
  -- Validate truoc khi ghi bat ky du lieu phu nao
  FOR v_item IN
  ...
  LOOP
    ...
    IF NOT v_is_test AND v_item.loai_item = 'the_lieu_trinh' THEN
      SELECT * INTO v_card FROM the_lieu_trinh WHERE id = v_item.the_lieu_trinh_id FOR UPDATE;
      IF NOT FOUND THEN
  ...
  -- Ghi nhan_vien_thu_nhap (ca test lan that deu ghi, phan biet bang is_test)
  ...
  LOOP
    IF v_item.nhan_vien_id IS NOT NULL THEN
      ...
      IF v_tour_amount > 0 THEN
        ...
        DO UPDATE SET so_tien = EXCLUDED.so_tien, is_test = EXCLUDED.is_test, updated_at = now();
      END IF;
  ...
  -- Side effects that (bo qua khi test mode)
  IF NOT v_is_test THEN
    -- Sinh doanh_thu (chi 3 PTTT vao dong tien)
    FOR v_payment IN
    ...
    FOR v_item IN
      SELECT * FROM don_hang_chi_tiet WHERE don_hang_id = p_don_hang_id ORDER BY created_at
    LOOP
      -- San pham: tru kho + ghi giao dich
      ...
      -- The lieu trinh cu: tru buoi + lich su
      ...
      -- The moi: tao record the_lieu_trinh
      IF v_item.loai_item = 'the_moi' THEN
        v_so_buoi_mua  := COALESCE((v_item.meta->>'soBuoiMua')::integer, v_item.so_luong, 0);
        v_so_buoi_tang := COALESCE((v_item.meta->>'soBuoiTang')::integer, 0);
        v_so_buoi_tong := v_so_buoi_mua + v_so_buoi_tang;
        ...
        INSERT INTO the_lieu_trinh (
          khach_hang_id, don_hang_id, dich_vu_id, nhan_vien_ban_id, ten_dich_vu,
          so_buoi_tong, so_buoi_da_dung, so_buoi_con_lai, gia_tri_the,
          ...
          v_so_buoi_tong, 0, v_so_buoi_tong, v_gia_tri_the,
          v_dh.ngay,
    ...
    IF v_con_no > 0 AND v_dh.khach_hang_id IS NOT NULL THEN
      ...
      SELECT
        ...
        COALESCE((SELECT so_du_con_lai FROM cong_no_khach_hang
                  WHERE khach_hang_id = v_dh.khach_hang_id ORDER BY created_at DESC LIMIT 1), 0) + v_con_no,
        v_dh.ngay, 'Phat sinh tu don ' || v_dh.ma_don;
    END IF;
...
-- ── 021 FINAL: pos_void_order (dao nguoc day du) ───────────
...
BEGIN
  ...
  -- Bao ve: the moi da dung buoi → can doi soat thu cong
  IF NOT v_is_test AND EXISTS (
    ...
    RETURN jsonb_build_object(
      'success', false,
      'error', 'The moi cua don nay da duoc su dung, can doi soat thu cong truoc khi huy'
    );
  END IF;

  -- Xoa thu nhap nhan vien
  DELETE FROM nhan_vien_thu_nhap WHERE don_hang_id = p_don_hang_id;
  ...
  IF NOT v_is_test THEN
    ...
    FOR v_item IN
      SELECT * FROM don_hang_chi_tiet
      WHERE don_hang_id = p_don_hang_id AND loai_item = 'san_pham'
    LOOP
      UPDATE kho_san_pham SET ton_kho = ton_kho + v_item.so_luong WHERE id = v_item.san_pham_id;
    ...
    FOR v_item IN
      SELECT * FROM don_hang_chi_tiet
      WHERE don_hang_id = p_don_hang_id AND loai_item = 'the_lieu_trinh'
    LOOP
      UPDATE the_lieu_trinh SET
  ...
  UPDATE don_hang SET trang_thai = 'huy', con_no = 0, updated_at = now()
  WHERE id = p_don_hang_id;
...
-- ── GRANT permissions ────────────────────────────────────────
GRANT EXECUTE ON FUNCTION pos_finalize_order TO authenticated;
GRANT EXECUTE ON FUNCTION pos_void_order     TO authenticated;
GRANT ALL ON TABLE nhan_vien_thu_nhap        TO authenticated;
